...
## data.xlsx
This file contains multiple sheets with the data and metadata for the requested dataset. The sheets contained in this workbook are:

### Data
This sheet contains a list of data records of the requested dataset.

Sheet structure:

| Column Name | Description                                                                                                                                                                                                                                                               |
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| indicatorId | The indicator code of the measured value. See `Indicators` sheet for the definition of the disaggregations (match via `indicatorId` field).                                                                                                                               |
| geoUnit     | GeoUnit id (country or region id) of the measured value. GeoUnit id (country or region id) of the measured value. See [geo unit definitions endpoint](https://api.uis.unesco.org/api/public/documentation/operations/listGeoUnits) for a list and names of all geo units. |
...
NOTE: The `Footnotes` sheet only exists when the `footnotes` parameter was set to `true` when requesting data. The `Indicators` and `Dissaggregation` sheets are optional and only exist if the `indicatorMetadata` parameter was set to `true` when requesting the data.

### Footnotes (optional)
Optional sheet, only exists if the `footnotes` parameter is set to `true` when fetching data.

This sheet contains a list of all data points level footnotes of the requested dataset. Matching of footnotes to data points is done via the `indicatorId`, `geoUnit`, and `year` fields combined.
...
### Indicators (optional)
Optional sheet, only exists if the `indicatorMetadata` parameter is set to `true` when fetching data.

This sheet contains a list of all indicators contained in the dataset.
...
| disaggregations                   | List of disaggregations (comma-separated) associated with this indicator. See `Disaggregations` sheet for the definition of the disaggregations (match via `code` column). |
| glossaryTerms                     | List of glossary terms associated with this indicator. See `Glossary Terms` sheet for the definition of the disaggregations (match via `termId` column).                   |
...
### Disaggregations (optional)
Optional sheet, only exists if the `indicatorMetadata` parameter is set to `true` when fetching data.

This sheet contains a list of all disaggregations contained used by any indicator in the dataset.
...
### Glossary Terms (optional)
Optional sheet, only exists if the `indicatorMetadata` parameter is set to `true` when fetching data.

This sheet contains a list of all glossary terms associated with any indicator or disaggregation in the dataset.
